...
import { ColorKey, VariantType, QuickActionItem } from '../../types/index';
...
const QuickActionCard: React.FC<QuickActionCardProps> = ({ action, theme, getColorClasses }) => {
  ...
  return (
    <motion.button
      className={`p-4 rounded-xl border transition-all duration-300 flex flex-col items-center space-y-2 group backdrop-blur-sm ${theme === 'dark'
    ...
    >
      <div
        className={`p-2 rounded-lg transition-all duration-300 group-hover:scale-110 ${getColorClasses(action.color, 'bg')
